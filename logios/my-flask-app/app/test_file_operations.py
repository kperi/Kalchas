...
class TestFileOperations(unittest.TestCase):
    ...
    def test_get_upload_root(self):
        self.assertEqual(
            file_operations.get_upload_root(MOCK_APP_CONFIG), "/tmp/test_uploads"
        )

    def test_get_user_dir(self):
        expected = os.path.join("/tmp/test_uploads", MOCK_USER_ID)
        self.assertEqual(
            file_operations.get_user_dir(MOCK_APP_CONFIG, MOCK_USER_ID), expected
        )

    def test_get_document_dir(self):
        expected = os.path.join("/tmp/test_uploads", MOCK_USER_ID, MOCK_FOLDER_NAME)
        self.assertEqual(
            file_operations.get_document_dir(
                MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
            ),
            expected,
        )
    ...
    def test_get_toocr_dir(self):
        expected = os.path.join(
            "/tmp/test_uploads", MOCK_USER_ID, MOCK_FOLDER_NAME, "TOOCR"
        )
        self.assertEqual(
            file_operations.get_toocr_dir(
                MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
            ),
            expected,
        )
    ...
    def test_get_page_segments_dir(self):
        ...
        self.assertEqual(
            file_operations.get_page_segments_dir(
                MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME, MOCK_PAGE_ID_BASE
            ),
            expected,
        )
    ...
    def test_get_combined_segments_parent_dir(self):
        expected = os.path.join(
            "/tmp/test_uploads", MOCK_USER_ID, MOCK_FOLDER_NAME, "TOOCR", "segments"
        )
        self.assertEqual(
            file_operations.get_combined_segments_parent_dir(
                MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
            ),
            expected,
        )
    ...
    def test_get_cropped_images_dir(self):
        expected = os.path.join(
            "/tmp/test_uploads", MOCK_USER_ID, MOCK_FOLDER_NAME, "cropped"
        )
        self.assertEqual(
            file_operations.get_cropped_images_dir(
                MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
            ),
            expected,
        )
    ...
    def test_get_segment_image_file_details_success_numeric_id(
        self, mock_exists, mock_isdir
    ):
        mock_isdir.return_value = True
        mock_exists.return_value = True
        segment_id_str = "1"
        expected_dir = file_operations.get_page_segments_dir(
            MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME, MOCK_PAGE_ID_BASE
        )
        ...
        mock_exists.assert_called_once_with(
            os.path.join(expected_dir, expected_filename)
        )

    @patch("app.file_operations.os.path.isdir")
    @patch("app.file_operations.os.path.exists")
    def test_get_segment_image_file_details_success_direct_filename(
        self, mock_exists, mock_isdir
    ):
        mock_isdir.return_value = True
        mock_exists.return_value = True  # Mock that direct filename.png exists
        segment_id_str = (
            "my_image.png"  # This will fail int() but then be used directly
        )
        expected_dir = file_operations.get_page_segments_dir(
            MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME, MOCK_PAGE_ID_BASE
        )
    ...
    @patch("app.file_operations.os.path.isdir")
    @patch("app.file_operations.os.path.exists")
    def test_get_segment_image_file_details_file_not_found(
        self, mock_exists, mock_isdir
    ):
        mock_isdir.return_value = True
    ...
    @patch("app.file_operations.os.path.isdir")
    @patch("app.file_operations.os.path.exists")
    def test_get_segment_image_file_details_invalid_id_format(
        self, mock_exists, mock_isdir
    ):
        mock_isdir.return_value = True
    ...
    def test_fetch_page_segments_data_success(
        self, mock_json_load, mock_file_open, mock_listdir, mock_isdir
    ):
        mock_isdir.return_value = True
        ...
        result = file_operations.fetch_page_segments_data(
            MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME, MOCK_PAGE_ID_BASE
        )
    ...
    @patch("app.file_operations.open", new_callable=mock_open)
    @patch("app.file_operations.json.load")
    def test_fetch_page_segments_data_json_load_error(
        self, mock_json_load, mock_file_open, mock_listdir, mock_isdir
    ):
        mock_isdir.return_value = True
    ...
    @patch("app.file_operations.os.path.isdir")
    @patch("app.file_operations.os.listdir")
    def test_resolve_segment_image_filename_from_index_success(
    ...
    def test_resolve_segment_image_filename_from_index_out_of_bounds_fallback_direct(
        self, mock_exists, mock_listdir, mock_isdir
    ):
        mock_isdir.return_value = True
    ...
    @patch("app.file_operations.os.listdir")
    @patch("app.file_operations.os.path.exists")
    def test_resolve_segment_image_filename_from_index_value_error_fallback_direct(
        self, mock_exists, mock_listdir, mock_isdir
    ):
        mock_isdir.return_value = True
    ...
    @patch("app.file_operations.open", new_callable=mock_open)
    def test_save_uploaded_cropped_image_success_first_crop(
        self, mock_file, mock_b64decode, mock_listdir, mock_makedirs
    ):
    ...
    @patch("app.file_operations.base64.b64decode")
    @patch("app.file_operations.open", new_callable=mock_open)
    def test_save_uploaded_cropped_image_success_next_crop(
        self, mock_file, mock_b64decode, mock_listdir, mock_makedirs
    ):
    ...
    @patch("app.file_operations.base64.b64decode")
    def test_save_ocr_processed_data_list_payload_with_image(
        self, mock_b64decode, mock_json_dump, mock_file, mock_makedirs
    ):
        ocr_payload = [
            {
                "text": "Segment 1 text",
    ...
    @patch("app.file_operations.open", new_callable=mock_open)
    @patch("app.file_operations.json.dump")
    def test_save_ocr_processed_data_status_dict_payload(
        self, mock_json_dump, mock_file, mock_makedirs
    ):
    ...
    def test_copy_files_to_ocr_folder_with_crops(
        self, mock_copy, mock_listdir, mock_path_exists, mock_makedirs
    ):
        source_doc_dir = file_operations.get_document_dir(
            MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
        )
        source_cropped_dir = file_operations.get_cropped_images_dir(
            MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
        )
        dest_toocr_dir = file_operations.get_toocr_dir(
            MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
        )

        mock_path_exists.return_value = (
            True  # Both source_cropped_dir and source_doc_dir exist
        )
        ...
        def listdir_side_effect(path):
            ...
            if path == source_cropped_dir:
                return [
                    "001_cropped_001.png",
                    "001_cropped_002.png",
                ]  # Cropped files for page 001
            return []

        mock_listdir.side_effect = listdir_side_effect

        success, count = file_operations.copy_files_to_ocr_folder(
            MOCK_APP_CONFIG, MOCK_USER_ID, MOCK_FOLDER_NAME
        )
        ...
        expected_calls = [
            call(
                os.path.join(source_cropped_dir, "001_cropped_001.png"),
                os.path.join(dest_toocr_dir, "page_001_crop_001.png"),
            ),
            call(
                os.path.join(source_cropped_dir, "001_cropped_002.png"),
                os.path.join(dest_toocr_dir, "page_001_crop_002.png"),
            ),
            call(
                os.path.join(source_doc_dir, "002.png"),
                os.path.join(dest_toocr_dir, "page_002.png"),
            ),
        ]
